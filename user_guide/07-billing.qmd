...
Set `BILLING_ENABLED=0` to hide billing routes and UI entirely (useful when a fork does not need payments yet).
...
1. Set `BILLING_ENABLED=1` and fill in `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID`, and `BASE_URL=http://localhost:8000` in `.env`.
...
2. Claims the event ID (idempotent processing)
3. Updates `OrganizationBilling` for the matching `organization_id` metadata
4. Returns `500` on handler failure so Stripe retries
