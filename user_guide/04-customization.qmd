...
Organization-scoped Stripe billing lives in `routers/app/billing.py` and `utils/app/billing.py`. Set `BILLING_ENABLED=1` and configure Stripe env vars to turn it on. See the [billing guide](https://promptlytechnologies.com/fastapi-jinja2-postgres-webapp/user-guide/billing.html) for Dashboard setup, webhooks, fork helpers (`org_has_active_subscription`), and a manual end-to-end checklist.
